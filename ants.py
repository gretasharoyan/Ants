...
class ThrowerAnt(Ant):
    """ThrowerAnt throws a leaf each turn at the st Bee in its range."""
    ...
    def nearest_bee(self, beehive):
        ...
        place_with_bee = self.place
        current = 0

        while place_with_bee != beehive:
            if place_with_bee.bees and current <= self.max_range and current >= self.min_range:
                return rANTdom_else_none(place_with_bee.bees)
            else:
                current+=1
                place_with_bee = place_with_bee.entrance
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        copy_list_bees = self.place.bees[:]

        if self.armor - amount <= 0:
            for bee in copy_list_bees:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if not self.contained_ant and  not isinstance(other, ContainerAnt):
            return True
        else:
            return False
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            check = self.place.exit
            while check:

                if check.ant:
                    if isinstance(check.ant, ContainerAnt):
                        if check.ant not in self.ants_lists:
                            check.ant.damage *= 2
                        if check.ant.contained_ant and check.ant.contained_ant not in self.ants_lists:
                            check.ant.contained_ant.damage *= 2

                            self.ants_lists.append(check.ant.contained_ant)
                        self.ants_lists.append(check.ant)
                    if check.ant not in self.ants_lists:
            ...
            #self.place.bee.reduce_armor(self.damage)
        # END Problem 13
...
def make_slow(action, bee):
    ...
    def new_action_method(gamestate):
